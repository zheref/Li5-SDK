...
open class CardStackUICollectionViewLayout: UICollectionViewLayout, UIGestureRecognizerDelegate {
...
//    var delegate: TisprCardStackViewControllerDelegate?
    ...
    @IBInspectable fileprivate let coefficientOfRotation: Double = 0.25
    
    @IBInspectable fileprivate let angleOfRotationForNewCardAnimation: CGFloat = 0.25
    
    @IBInspectable fileprivate let verticalOffsetBetweenCardsInTopStack: CGFloat = 35
    @IBInspectable fileprivate let horizontalOffsetBetweenCardsInTopStack: CGFloat = 15.0
    @IBInspectable fileprivate let centralCardYPosition: CGFloat = 35
    ...
    @IBInspectable fileprivate let minimumXPanDistanceToSwipe: CGFloat = 100
    @IBInspectable fileprivate let minimumYPanDistanceToSwipe: CGFloat = 60
    ...
    fileprivate func layoutAttributesForTopStackItemWithInitialAttributes(_ attributes: UICollectionViewLayoutAttributes) -> UICollectionViewLayoutAttributes {
        ...
        let cardFrame = CGRect(x: xPosition, y: CGFloat(yPosition), width: cardWidth, height: cardHeight)
    ...
    internal func handleTap(_ sender: UITapGestureRecognizer) {
        if (sender.state == .ended) {
            let initialPanPoint = sender.location(in: collectionView)
            if let indexPath = collectionView?.indexPathForItem(at: initialPanPoint) {
                moveCard(indexPath)
            }
        }
    }
    ...
    fileprivate func moveCard(_ indexPath: IndexPath) {
        if (index + 1 == collectionView!.numberOfItems(inSection: 0))
        {
            while ( index != 0 )
            {
