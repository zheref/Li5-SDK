...
+ (UIBezierPath *)stars:(NSUInteger)numberOfStars shapeInFrame:(CGRect)originalFrame
{
    ...
    for (int i=0; i < numberOfStars; i++) {
        // get the star shape
        UIBezierPath* startPath = [ShapesHelper starShape:babyFrame];
...
{
    UIBezierPath *categoryPath = [UIBezierPath bezierPath];
    
    // Set the starting point of the shape.
    [categoryPath moveToPoint:CGPointMake(0.0, height/2)];
    
    // Draw the lines.
    [categoryPath addLineToPoint:CGPointMake(width/4, 0.0)];
    [categoryPath addLineToPoint:CGPointMake(width + width/4, 0.0)];
    [categoryPath addLineToPoint:CGPointMake(width + 2*width/4, height/2)];
    [categoryPath addLineToPoint:CGPointMake(width + width/4, height)];
    [categoryPath addLineToPoint:CGPointMake(width/4, height)];
    [categoryPath closePath];
...
+ (UIBezierPath *) rectangleWithWidth:(NSUInteger)width andHeight:(NSUInteger)height
{
    ...
    // Draw the lines.
    [aPath addLineToPoint:CGPointMake(width, 0.0)];
